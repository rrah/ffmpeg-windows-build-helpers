...
check_missing_packages () {
  ...
  if [[ -n "${missing_packages[@]}" ]]; then
    ...
    echo "for ubuntu: $ sudo apt-get install subversion texinfo g++ bison flex cvs yasm automake libtool autoconf gcc cmake git make pkg-config zlib1g-dev mercurial curl" 
    exit 1
  fi
